<?php
	if (isset($_POST["forgotPass"])) {
		$connection = new mysqli("localhost", "root", "", "tuition");
		
		$email = $connection->real_escape_string($_POST["email"]);
		
		$data = $connection->query("SELECT ID FROM users WHERE EMAILADD='$email'");
		...
		if ($data->num_rows > 0) {
			...
			$connection->query("UPDATE users SET token='$str', expire = DATE_ADD(NOW(), INTERVAL 5 MINUTE) WHERE EMAILADD='$email'");
